...
def ais_fdr(settings=None, *results):
    ...
    for process in results_comb.processes_analysed:
        if results_comb._single_process[process].ais_sign:
            pval = np.append(pval,
                             results_comb._single_process[process].ais_pval)
    ...
    if (1 / min(n_perm)) > thresh[0]:
        print('WARNING: Number of permutations ('
              'n_perm_max_seq'
              ') for at '
    ...
    if correct_by_target:  # whole target
        for target in results_comb.targets_analysed:
            if results_comb._single_target[target].omnibus_sign:
                ...
                n_perm = np.append(n_perm,
                                   results_comb.settings.n_perm_omnibus)
        ...
        for target in results_comb.targets_analysed:
            if results_comb._single_target[target].omnibus_sign:
                ...
                n_perm = np.append(n_perm,
                                   results_comb.settings.n_perm_max_seq)

    if pval.size == 0:
        print('No links in final results ...')
        results_comb._add_fdr(fdr=None,
                              alpha=alpha,
...
def omnibus_test(analysis_setup, data):
    ...
    cond_source_realisations = (
        analysis_setup._selected_vars_sources_realisations)
    ...
    if (analysis_setup._cmi_estimator.is_analytic_null_estimator()
        ...
        surr_cond_real = _get_surrogates(data, analysis_setup.current_value,
        ...
        surr_distribution = analysis_setup._cmi_estimator.estimate_parallel(
            ...
            var2=analysis_setup._current_value_realisations,
            conditional=cond_target_realisations)
    [significance, pvalue] = _find_pvalue(statistic, surr_distribution, alpha,
                                          'one_bigger')
    if analysis_setup.settings['verbose']:
        if significance:
            print(' -- significant\n')
...
def max_statistic(analysis_setup,
                  data,
                  candidate_set,
                  te_max_candidate,
                  conditional=None):
...
def max_statistic_sequential(analysis_setup, data):
    ...
    surr_table = np.zeros(
        (len(analysis_setup.selected_vars_sources), n_permutations))
    ...
    for idx_c, candidate in enumerate(analysis_setup.selected_vars_sources):
        [conditional_realisations_current, candidate_realisations_current
        ...
        if conditional_realisations_current is None:
            ...
            conditional_realisations[
                i_1:i_2, ] = conditional_realisations_current
            re_use = ['var2']
        ...
        if (analysis_setup._cmi_estimator.is_analytic_null_estimator()
            ...
            surr_candidate_realisations = _get_surrogates(
                data, analysis_setup.current_value, [candidate],
                n_permutations, analysis_setup.settings)
            try:
                surr_table[idx_c, :] = (
...
def max_statistic_sequential_bivariate(analysis_setup, data):
    ...
    significance = np.zeros(len(
        analysis_setup.selected_vars_sources)).astype(bool)
    ...
    for source in significant_sources:
        ...
        for idx_c, candidate in enumerate(source_vars):
            ...
            temp_cand = data.get_realisations(analysis_setup.current_value,
                                              [candidate])[0]
...
def min_statistic(analysis_setup,
                  data,
                  candidate_set,
                  te_min_candidate,
                  conditional=None):
...
def mi_against_surrogates(analysis_setup, data):
    ...
    if (analysis_setup._cmi_estimator.is_analytic_null_estimator()
        ...
        analysis_setup.settings['analytical_surrogates'] = False
        surr_realisations = _get_surrogates(data, analysis_setup.current_value,
                                            [analysis_setup.current_value],
                                            n_perm, analysis_setup.settings)
...
def unq_against_surrogates(analysis_setup, data):
    ...
    target_realisations = data.get_realisations(
        analysis_setup.current_value, [analysis_setup.current_value])[0]
    source_1_realisations = data.get_realisations(
        analysis_setup.current_value, [analysis_setup.sources[0]])[0]
    source_2_realisations = data.get_realisations(
        analysis_setup.current_value, [analysis_setup.sources[1]])[0]
    ...
    surr_realisations = _get_surrogates(data, analysis_setup.current_value,
                                        [analysis_setup.sources[0]], n_perm,
                                        analysis_setup.settings)
    ...
    surr_realisations = _get_surrogates(data, analysis_setup.current_value,
                                        [analysis_setup.sources[1]], n_perm,
                                        analysis_setup.settings)
...
def syn_shd_against_surrogates(analysis_setup, data):
    ...
    surr_realisations = _get_surrogates(data, analysis_setup.current_value,
                                        [analysis_setup.current_value], n_perm,
                                        analysis_setup.settings)
...
def check_n_perm(n_perm, alpha):
    ...
    if not 1.0 / n_perm < alpha:
        raise RuntimeError('The number of permutations {0} is to small to test'
                           ...
                           'permutations must be greater than 1/alpha.'.format(
                               n_perm, alpha))


def _create_surrogate_table(analysis_setup,
                            data,
                            idx_test_set,
                            n_perm,
                            conditional=None):
    ...
    for candidate in idx_test_set:
        if (analysis_setup._cmi_estimator.is_analytic_null_estimator()
            ...
            analysis_setup.settings['analytical_surrogates'] = False
            surr_candidate_realisations = _get_surrogates(
                data, analysis_setup.current_value, [candidate], n_perm,
                analysis_setup.settings)
            surr_table[idx_c, :] = (
                analysis_setup._cmi_estimator.estimate_parallel(
                    n_chunks=n_perm,
...
def _sort_table_max(table):
    ...
    for permutation in range(0, table.shape[1]):
        table_sorted[:,
                     permutation] = utils.sort_descending(table[:,
                                                                permutation])
    return table_sorted
...
def _find_pvalue(statistic, distribution, alpha, tail):
    ...
    if tail == 'one_bigger' or tail == 'one':
        ...
        raise ValueError(('Unkown value for '
                          'tail'
                          ', should be '
                          'one'
                          ', '
...
def _get_surrogates(data, current_value, idx_list, n_perm, perm_settings):
    ...
    surrogates = np.empty(
        (n_realisations * n_perm, len(idx_list))).astype(data.data_type)
    ...
    # surrogate data generated by fourier transform
    if perm_type == "RP" or perm_type == "AAFT":
        # print(f"Generate surrogate data by using {perm_type} method")
        for _ in range(n_perm):
            surrogates[i_1:i_2, ] = data.Fourier_surrogate(
                current_value, idx_list, perm_settings)[0]
            i_1 = i_2
            i_2 += n_realisations
        return surrogates
    ...
    if permute_in_time:
        ...
        assert _sufficient_replications(
            data, n_perm), ('Not enough replications for surrogate creation.')
        for perm in range(n_perm):
            surrogates[i_1:i_2, ] = data.permute_replications(
                current_value, idx_list)[0]
            i_1 = i_2
            i_2 += n_realisations
    return surrogates
...
def _generate_spectral_surrogates(data, scale, n_perm, perm_settings):
    ...
    # Allocate memory for surrogates
    surrogates = np.empty(
        (data.n_samples, data.n_replications, n_perm)).astype(data.data_type)
    permute_in_time = perm_settings['permute_in_time']
    # Generate surrogates by permuting over replications if possible (no.
    # replications needs to be sufficient); else permute samples over time.
    if permute_in_time:
...
def _check_permute_in_time(analysis_setup, data, n_perm):
    ...
    if (not analysis_setup.settings['permute_in_time']
            and not _sufficient_replications(data, n_perm)):
